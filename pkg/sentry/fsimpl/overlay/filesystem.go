...
func (fs *filesystem) RenameAt(ctx context.Context, rp *vfs.ResolvingPath, oldParentVD vfs.VirtualDentry, oldName string, opts vfs.RenameOptions) error {
	...
	cleanupRecreateWhiteouts := func() {
		...
		for whiteoutName, whiteoutUpper := range whiteouts {
			...
			if err := fs.createWhiteout(ctx, vfsObj, &vfs.PathOperation{
				...
				ctx.Warningf("Unrecoverable overlayfs inconsistency: failed to recreate deleted whiteout after RenameAt failure: %v", err)
			}
		}
	}
	if renamed.isDir() {
	...
	if err := fs.createWhiteout(ctx, vfsObj, &oldpop); err != nil {
		ctx.Warningf("Unrecoverable overlayfs inconsistency: failed to create whiteout at origin after RenameAt: %v", err)
		return err
	}
	if renamed.isDir() {
		if err := vfsObj.SetXattrAt(ctx, fs.creds, &newpop, &vfs.SetXattrOptions{
			...
			ctx.Warningf("Unrecoverable overlayfs inconsistency: failed to make renamed directory opaque: %v", err)
			return err
		}
	}
