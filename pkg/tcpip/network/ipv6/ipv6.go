...
func (e *endpoint) removePermanentEndpointLocked(r stack.AddressEndpoint, allowSLAACInvalidation bool) *tcpip.Error {
	...
	if err := e.mu.addressableEndpoint.RemoveAddress(addr.Address); err != nil {
		return err
	}
...
func (e *endpoint) RemoveAllAddresses() *tcpip.Error {
	e.mu.Lock()
	defer e.mu.Unlock()
	return e.removeAllAddressesLocked()
}

func (e *endpoint) removeAllAddressesLocked() *tcpip.Error {
	var err *tcpip.Error
	// We know this is safe because the returned endpoints is only held
	// for the duration of this function.
	for _, r := range e.mu.addressableEndpoint.AllEndpointsUnsafe() {
		...
		case stack.PermanentTentative, stack.Permanent:
			if tempErr := e.removePermanentEndpointLocked(r, true); tempErr != nil && err == nil {
				err = tempErr
			}
		}
	}
